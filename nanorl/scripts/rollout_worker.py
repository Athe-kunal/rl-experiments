...
it over localhost HTTP:
  ...
  - POST /update_weights_start {names, dtype_names, shapes, packed, is_checkpoint_format}
  - POST /update_weights_finish {}
  - POST /reload     {model_path} (legacy fallback)

The trainer keeps semantics strict by:
  1. generating step-t rollouts from weights W_t
  2. updating the policy to W_{t+1}
  3. syncing W_{t+1} to this worker in-place
...
def main():
    ...
    parser.add_argument(
        ...
        default="",
        choices=["", "nccl", "ipc"],
        help="Enable in-place weight transfer with the chosen backend.",
    )
